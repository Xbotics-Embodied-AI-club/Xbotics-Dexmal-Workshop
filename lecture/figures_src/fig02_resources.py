"""图 2：教学仓与它安装时自动取来的三个依赖，以及权重和数据从哪里来。"""
...
BOXES = {
    ...
    "sim": (0.36, 0.80, 0.28, "SO-101 仿真器\n三个抓放场景", "#eef3f8"),
    "lerobot": (0.70, 0.80, 0.28, "LeRobot\n仿真与真机的统一接口", "#eef3f8"),
...
LABELS = ("uv sync 自动安装", "download --weights", "download --data")
...
def main() -> None:
    ...
    for key in BOXES:
        box(ax, key)
    top = 0.44 + H
    for key in ("dexbotic", "sim", "lerobot"):
        x, y, w = BOXES[key][:3]
