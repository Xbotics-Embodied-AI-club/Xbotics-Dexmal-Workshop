"""下载权重与训练数据，落到工作区目录（默认 `~/so101_workspace`，改 `SO101_ROOT` 换位置）。
    ...
    python -m dexmal_workshop.download --weights    # 两份发布的权重 + DW0.5 推理用的基座组件
    python -m dexmal_workshop.download --data       # 训练数据 3698 集（仿真 1498 + 真机 2200）
...
def data() -> None:
    ...
    print(f"训练数据就位：{root}；下一步 python -m dexmal_workshop.prepare_data")
