"""把下载好的仿真与真机数据转成 DM0.5 / DW0.5 训练用的统一格式（dexdata）。
    ...
    python -m dexmal_workshop.prepare_data

输入（`dexmal_workshop.download --data` 下好的位置）：
...
def main() -> int:
    ...
    for directory in (sim, real):
        if not directory.is_dir():
            raise SystemExit(f"数据不在 {directory}；先运行 python -m dexmal_workshop.download --data")
